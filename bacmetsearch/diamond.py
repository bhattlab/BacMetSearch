...
def run_diamond(protein_fasta_path, outfile, threads, max_target_seqs, min_percent_identity):
    command = '{0} blastp --id {5} -k {6} --query {1} --out {2}.dmd.tsv --outfmt 6 qseqid sseqid qlen slen pident ' \
        ...
        DIAMOND_PATH, protein_fasta_path, outfile, PHYLOPHLAN_MARKER_PATH, threads, min_percent_identity, max_target_seqs)
